class Contract(object):
    ...
    def total_value(self):
        # This will try final price with monthly_payments if it fails it will
        # use length in months before returning the final price
        try:
            final_price = self.vehicle.sale_price() + (
                self.vehicle.interest * self.monthly_payments *
                self.vehicle.sale_price() / 100)
        except:
            final_price = self.vehicle.sale_price() + (
                self.vehicle.sale_price() * self.vehicle.lease_mulitiplier /
                self.length_in_months)
        return final_price - (final_price * self.discount)

    def monthly_value(self):
        # This will convert monthly_payments or length_in_months to months so
        # we can return monthly value
        try:
